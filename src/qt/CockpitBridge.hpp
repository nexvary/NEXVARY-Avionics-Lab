...
public:
    ...
    QVariantList tiles() const; QVariantList sensorRows() const; QVariantList eventRows() const;
    QStringList annunciators() const; QString scenario() const; qulonglong tick() const noexcept;
    int recordedFrames() const noexcept; int activeAlertCount() const noexcept; int sensorCount() const noexcept; int eventCount() const noexcept;
    bool replayMode() const noexcept; int replayIndex() const noexcept; int replayMaximum() const noexcept;
    QString language() const; bool rtl() const noexcept; QStringList scenarios() const;
    Q_INVOKABLE void step(); Q_INVOKABLE void resetLab(); Q_INVOKABLE void setScenario(const QString& name);
    Q_INVOKABLE void setLanguage(const QString& code); Q_INVOKABLE QString text(const QString& key) const;
    Q_INVOKABLE void setReplayMode(bool enabled); Q_INVOKABLE void seekReplay(int index);
signals: void dataChanged(); void languageChanged();
private:
    void refresh(const LabSnapshot& snapshot); void showReplayFrame();
    AvionicsLab lab_; CockpitViewModel viewModel_; UiLanguage language_{UiLanguage::English}; LabSnapshot snapshot_;
    QVariantList tiles_; QVariantList sensorRows_; QVariantList eventRows_; QStringList annunciators_;
    bool replayMode_{false}; int replayIndex_{0};
};
}
