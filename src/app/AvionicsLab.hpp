...
struct LabSnapshot {std::uint64_t tick{0};std::chrono::milliseconds simTime{0};ScenarioKind scenario{ScenarioKind::Nominal};std::map<std::string,SensorSample> sensors;std::vector<HealthIssue> issues;std::vector<Alert> alerts;};
class AvionicsLab {
public:
...
private:
 SensorBus bus_;EventLog log_;SystemHealth health_;AlertManager alerts_;TelemetryRecorder recorder_;FaultInjector scenarioFaults_;FaultInjector trainingFaults_;SimulationClock clock_;std::unique_ptr<SimulationProvider> provider_;std::size_t lastActiveAlerts_{0};
};
} // namespace nexvary::avionics
