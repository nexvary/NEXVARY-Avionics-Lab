...
Canvas {
    ...
    function drawGrid(c, w, h) {
        ...
        c.lineWidth = 1
        c.globalAlpha = .62
        for (var x = 22; x < w; x += 22) {
            c.beginPath(); c.moveTo(x, 0); c.lineTo(x, h); c.stroke()
        }
        for (var y = 22; y < h; y += 22) {
    ...
    function drawJet(c, w, h) {
        ...
        for (var y = .38; y <= .58; y += .05) {
            c.beginPath(); c.moveTo(w * .22, h * y); c.lineTo(w * .78, h * y); c.stroke()
        }
        c.globalAlpha = 1
    }
    ...
    function drawNodes(c, w, h) {
        ...
        for (var i = 0; i < positions.length; ++i) {
            ...
            var py = h * positions[i][1]
            var sc = stateColorAt(i)
