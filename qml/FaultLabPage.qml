...
Item {
    ColumnLayout { anchors.fill:parent; anchors.margins:18; spacing:14
        Rectangle {
            ...
            radius:10
            color:"#0a151a"
            border.color:cockpit.activeTrainingFaultCount>0?Theme.amber:Theme.green
        ...
        Rectangle { Layout.fillWidth:true; Layout.fillHeight:true; Layout.minimumHeight:240; radius:10; color:Theme.panel; border.color:Theme.border
            ColumnLayout { anchors.fill:parent; anchors.margins:14
                RowLayout { Layout.fillWidth:true
                    Text { text:cockpit.text("recent_events"); color:Theme.gold; font.bold:true; font.pixelSize:16; Layout.fillWidth:true }
                    Text { text:cockpit.eventCount+" "+cockpit.text("events"); color:Theme.muted; font.pixelSize:10 }
                }
                ListView { Layout.fillWidth:true; Layout.fillHeight:true; model:cockpit.eventRows; clip:true; spacing:4
                    delegate: Rectangle {
                        required property int index
                        required property var modelData
                        width:ListView.view.width; height:38; color:index%2?"#07141b":"#091820"
                        RowLayout { anchors.fill:parent; anchors.margins:7
                            Text { text:modelData.severity; color:modelData.severity==="FAULT"?Theme.red:(modelData.severity==="WARN"?Theme.amber:Theme.green); Layout.preferredWidth:70; font.bold:true }
