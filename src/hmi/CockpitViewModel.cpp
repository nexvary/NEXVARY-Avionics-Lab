...
CockpitPage CockpitViewModel::build(const LabSnapshot& snapshot) const {
    ...
    page.subtitle = std::string{"TRAINING / SIMULATION — "} + to_string(snapshot.scenario);
    addSensor(page, snapshot, "altitude_m", "ALTITUDE");
    addSensor(page, snapshot, "airspeed_kts", "AIRSPEED");
    ...
    addSensor(page, snapshot, "hydraulic_pressure_bar", "HYDRAULIC");
    addSensor(page, snapshot, "fuel_percent", "FUEL");

    for (const auto& alert : snapshot.alerts) {
        if (alert.active) page.annunciators.push_back(alert.subsystem + ": " + alert.detail);
    }
