...
enum class TwinState { Unknown, Nominal, Degraded, Fault };

struct TwinSubsystem {
    std::string id;
    TwinState state{TwinState::Unknown};
    std::size_t expectedChannels{0};
    std::size_t observedChannels{0};
...
struct DigitalTwinSnapshot {
    std::vector<TwinSubsystem> subsystems;
    std::size_t nominalCount{0};
...
public:
    [[nodiscard]] static DigitalTwinSnapshot build(
